...
;(process.env.CI ? it.skip : it)('typescript + toolchain', async function () {
  this.timeout(60000)
  ...
  await execa('pnpm', ['run', 'tc', 'migrate'], execaOpts)
  await execa('pnpm', ['run', 'tc', 'check'], execaOpts)
  await execa('pnpm', ['run', 'tc', 'build'], execaOpts)
})
